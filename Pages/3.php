...
<body>
<h2 style="color:white;">GROUP C STAGE</h2>
<?php
session_start();
...
if($result1->num_rows>0 && $result2->num_rows>0)
{
	while($row1=$result1->fetch_assoc() && $row2=$result2->fetch_assoc())
	{
		...
		if($winprobA-$winprobB>0.0001)
		{
			...
			$link->query($sql7);
			$sql8="update c1 set points+='3',goal_scored+='$gs1',goal_diff+='$gs1-$gs2' where tname='$var1'";
			$link->query($sql8);
			$sql9="update c1 set points+=0,goal_scored+=$gs2,goal_diff-=($gs1-$gs2) where tname='$var2'";
			$link->query($sql9);
		...
		else
		{
			...
			$link->query($sql7);
			$dd=$gs2-$gs1;
			$sql8="update c1 set points='3',goal_scored='$gs2',goal_diff='$dd' where tname='$var2'";
			$link->query($sql8);
			$sql9="update c1 set points+=0,goal_scored+=$gs1,goal_diff-=($gs2-$gs1) where tname='$var1'";
			$link->query($sql9);
