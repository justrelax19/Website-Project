...
<body>
<h2 style="color:white;">SEMI FINAL MATCHES</h2>
<?php
session_start();
...
if($result1->num_rows>0 && $result2->num_rows>0)
{
	while($row1=$result1->fetch_assoc() && $row2=$result2->fetch_assoc())
	{
		...
		if($winprobA-$winprobB>0.0001)
		{
			...
			$link->query($sql7);
			$sql8="insert into final values ('$var1')";
			$link->query($sql8);
		...
		else
		{
			...
			$link->query($sql7);
			$sql8="insert into final values ('$var2')";
			$link->query($sql8);
...
else
{
	$_SESSION["match"]++;
	$d=$_SESSION["match"];
	echo "<div align='center'><a href='$d.php' target='names' style='color:white;text-decoration:none;'><h3>Final Match</h3></a></div>";
	$_SESSION["val"]=0;
}
//ob_end_clean();
?>
